<template>
  <div class="w_menu_form">
    <el-dialog :title="isAdd ? '新增菜单' : '编辑菜单'" :visible.sync="dialogVisible"
               width="540px" center :append-to-body="true" custom-class="menu-dialog">
      <el-form :model="form" ref="form" size="mini" label-width="80px" :rules="rulesForm">
        <el-form-item label="菜单图标" prop="svgIcon">
          <el-popover placement="bottom-start" width="400" trigger="click">
            ...
            <el-input slot="reference" type="text" v-model="form.svgIcon" placeholder="点击选择图标" readonly>
              <svg v-if="form.svgIcon" slot="prefix" class="icon el-input__icon icon-addMu" aria-hidden="true">
                <use :xlink:href="form.svgIcon.includes('#') ? form.svgIcon : '#'+form.svgIcon"></use>
              </svg>
        ...
        <el-form-item label="菜单排序" prop="id">
          <el-input-number type="number" v-model="sortId" controls-position="right"
                           autocomplete="off" placeholder="排序" :min="minSort">
          </el-input-number>
        </el-form-item>
        <el-form-item label="内部菜单" prop="iframe">
          <el-radio-group v-model="form.iframe">
            <el-radio v-for="item in iframeLabel" :label="item.label" :key="item.label+'ia'">{{ item.value }}</el-radio>
          </el-radio-group>
        </el-form-item>
