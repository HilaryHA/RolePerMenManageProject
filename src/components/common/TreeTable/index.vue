<template>
  ...
  <el-table :data="formatData" :row-style="showRow" v-bind="$attrs" :row-key="getRowKeys" class="tree_table">
    <!-- 表示children为空的父元素 -->
    ...
    <el-table-column v-for="(column, index) in columns" v-else :key="column.value" :label="column.text" :width="column.width">
      <template slot-scope="scope">
        ...
        <span v-if="iconShow(index, scope.row)" class="icon_tree" @click="toggleExpanded(scope.row)">
          <i v-if="!scope.row._expanded" class="el-icon-plus"></i>
          <i v-else class="el-icon-minus"></i>
        </span>
        <!-- column.value此处为name -->
...
<script>
  ...
  export default {
    ...
    props: { /*跳过eslint的检验*/
      ...
      evalFunc: Function,
      evalArgs: Array,
      expandAll: {  // 对应dept的index.vue中传递的值expand-all
        type: Boolean,
        default: false
      }
    },
    watch: {
      data: {
        handler (newValue, oldValue) {
          // console.log('========>oldValue初始为空值<==========',oldValue, newValue);
          // if ( newValue ) {
...
<style lang="scss" rel="stylesheet/scss">
  ...
  .tree_table {
    ...
    .el-table__expand-icon { // 去除默认的icon
      width: 0;
      i::before {
        content: '';
      }
